...
function TimedEvent(props) {
    ...
    return React.createElement(
        "div",
        {
            ...
            }, eventId: props.event.id, onClick: function onClick(event) {
    ...
    return React.createElement(
        ...
        React.createElement(
            "div",
            {
                className: "events",
                style: {
                    height: allDayEvents.length > 0 ? "calc(100% - " + (40 * allDayEvents.length + 63) + "px)" : "calc(100% - 95px)"
                },
                __source: {
                    fileName: _jsxFileName,
                    lineNumber: 454
...
function Day(props) {
    ...
    if (props.day < 1 || props.day > maxDays) {
        ...
        if (finalEvent_elements.length * 20 > props.dayHeight - 20) {
            finalEvent_elements = finalEvent_elements.slice(0, Math.abs(Math.floor(props.dayHeight / 20) - 2) >= 1 ? Math.abs(Math.floor(props.dayHeight / 20) - 2) : 1);
        }
        return React.createElement(
            "td",
            {
                ...
                onClick: function onClick(e) {
                    if (e.target.classList.contains("day") || e.target.classList.contains("day-events") || e.target.classList.contains("day-number") || e.target.classList.contains("incomplete-number")) {
                        ...
                        fetch("/events/" + dayDate.getFullYear() + "/" + (dayDate.getMonth() + 1) + "/" + dayDate.getDate()).then(function (response) {
                            response.json().then(function (data) {
                                ...
                                document.querySelector(".view-root > .view-area").querySelector('.timed-event:first-child').scrollIntoView(true);
                                document.querySelector(".view-root").classList.add("active");
                            });
                        });
                    }
